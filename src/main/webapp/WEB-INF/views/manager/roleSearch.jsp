				...
				<div class="main-content-inner">
					...
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						...
						<div class="nav-search" id="nav-search">
							<form class="form-search" action="${pageContext.request.contextPath }/role/toRoleSearch.do" method="post">
												...
												<div class="widget-header">
													...
													<div class="widget-toolbar no-border">
														<ul class="nav nav-tabs" id="myTab2">
															<li >
																<a data-toggle="tab" href="javascript:void(0)" onclick="window.location.href='${pageContext.request.contextPath }/role/toRoleAdd.do'">增加</a>
															</li>
												...
												<div class="widget-body">
													<div class="widget-main padding-12 no-padding-left no-padding-right">
														<div class="tab-content padding-4">
															...
															<button id="btnDeleteChkRole" class="btn btn-primary">删除勾选用户</button>
															     
																 <table class="table table-striped table-bordered align-center " style="table-layout:fixed;"> 
																    <tr class="bolder" >
																   ...
																   <c:forEach var="role" varStatus="status" items="${requestScope.page.data }">
																    <tr>
																	  ...
																	  <td style="word-wrap:break-word;">${role.role_powers}</td>
																	   <td><button class="btn btn-sm btn-primary" onclick="window.location.href='${pageContext.request.contextPath }/role/toRoleEdit.do?roleId=${role.role_id}'">更新</button>&nbsp;&nbsp;<button class="btn btn-sm btn-primary" onclick="window.location.href='${pageContext.request.contextPath }/role/deleteRole.do?roleId=${role.role_id}'" >删除</button></td>
																	</tr>
																   <tr>
																   </c:forEach>
																 </table>
																	  ...
																	  <ul class="pagination">
																	      ...
																	      <c:choose>
																	         <c:when test="${requestScope.page.previous==true }">
																	             <li>
																	               <a href="${pageContext.request.contextPath }/role/searchRole.do?index=${requestScope.page.index-1}&search_thing=${search_thing}" aria-label="Previous">
																	       ...
																	       <c:choose>
																	          ...
																	          <c:when test="${status.count==requestScope.page.index+1}">
							                                                         <%-- 注意事项：数据库的索引是从0开始的，而页面索引是从1开始的。索引数据索引=页面索引-1 --%>
																	      			<li class="active">
																	      			<a href="${pageContext.request.contextPath }/role/searchRole.do?index=${status.count-1}&search_thing=${search_thing}">${status.count }</a>
																	      			</li>										          
																	          </c:when>
																	          <c:otherwise>
																	                <li>
																	    ...
																	    <c:choose>
																	      <c:when test="${requestScope.page.next==true }">
																           <li>
																	      	  <a href="${pageContext.request.contextPath }/role/searchRole.do?index=${requestScope.page.index+1}&search_thing=${search_thing}" 
																	      	      aria-label="Next">
...
<![endif]-->
		...
		<!-- inline scripts related to this page -->
		       ...
		       $("#btnDeleteChkRole").click(function(){
		    	   //alert("--test--");
		    	   //1.获得选中的复选框的值
		    	   var chkRoles= $("input[name='roleId']:checked");
		    	   var params=chkRoles.serialize();
		    	   //2.将chkRole转换成发送的参数
