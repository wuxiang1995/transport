												...
												<div class="widget-body">
													<div class="widget-main padding-12 no-padding-left no-padding-right">
														<div class="tab-content padding-4">
															...
															<form id="from1" class="form-search" action="${pageContext.request.contextPath }/order/toOrderSearch.do" method="post">
															...
															<button id="btnDeleteChkorder" class="btn btn-primary">删除勾选用户</button>
															     
																 <table class="table table-striped table-bordered align-center "> 
																    <tr class="bolder" >
																	  <td><input id="chkAllorder" type="checkbox"></td>
																	  <td>订单编号</td>
																	  <td>客户姓名</td>
																	  <td>订单时间</td>
																	  <td>到付时间</td>
															      ...
															      <c:forEach var="order" varStatus="status" items="${requestScope.page.data }">
																    <tr>
																	  <td><input name="orderId" value="${order.order_id}" type="checkbox"></td>
																	  ...
																	  <td>${order.order_recieverphone}</td>
																	   <td><button class="btn btn-sm btn-primary" onclick="window.location.href='${pageContext.request.contextPath }/order/toOrderEdit.do?orderId=${order.order_id}'">编辑</button>&nbsp;&nbsp;<button class="btn btn-sm btn-primary" onclick="window.location.href='${pageContext.request.contextPath }/order/deleteOrder.do?orderId=${order.order_id}'" >删除</button></td>
																	</tr>
																   <tr>
																   </c:forEach>
																	  ...
																	  <ul class="pagination">
																	      ...
																	      <c:choose>
																	         <c:when test="${requestScope.page.previous==true }">
																	             <li>
																	               <a href="${pageContext.request.contextPath }/order/toOrderList.do?index=${requestScope.page.index-1}" aria-label="Previous">
																	       ...
																	       <c:choose>
																	          ...
																	          <c:when test="${status.count==requestScope.page.index+1}">
							                                                         <%-- 注意事项：数据库的索引是从0开始的，而页面索引是从1开始的。索引数据索引=页面索引-1 --%>
																	      			<li class="active">
																	      			<a href="${pageContext.request.contextPath }/order/toOrderList.do?index=${status.count-1}">${status.count }</a>
																	      			</li>										          
																	          </c:when>
																	          <c:otherwise>
																	                <li>
																	    ...
																	    <c:choose>
																	      <c:when test="${requestScope.page.next==true }">
																           <li>
																	      	  <a href="${pageContext.request.contextPath }/order/toOrderList.do?index=${requestScope.page.index+1}" 
																	      	      aria-label="Next">
...
<![endif]-->
		...
		<script type="text/javascript">
		       ...
		       $("#btnDeleteChkorder").click(function(){
		    	   //alert("--test--");
		    	   //1.获得选中的复选框的值
		    	   var chkorders= $("input[name='orderId']:checked");
		    	   var params=chkorders.serialize();
		    	   //2.将chkorder转换成发送的参数
		    	   window.location.href="${pageContext.request.contextPath }/order/deleteOrders.do?"+params;
		       });
